...
import emulation.emulmisc as emulmisc
import emulation.emultimer as emultimer
from . import nonportable
import sandbox.safe as safe # Used to get SafeDict
...
def _prepare_wrapped_functions_for_object_wrappers():
  ...
  """
  objects_tuples = [(LOCK_OBJECT_WRAPPER_INFO, lock_object_wrapped_functions_dict),
...
class Str(ValueProcessor):
  ...
  def check(self, val):
    if not _is_in(type(val), [str, str]):
...
class Int(ValueProcessor):
  ...
  def check(self, val):
    if not _is_in(type(val), [int, int]):
      raise RepyArgumentError("Invalid type %s" % type(val))
...
class NoneOrInt(ValueProcessor):
  ...
  def check(self, val):
    if val is not None and not _is_in(type(val), [int, int]):
      raise RepyArgumentError("Invalid type %s" % type(val))
...
class StrOrInt(ValueProcessor):
  ...
  def check(self, val):
    if not _is_in(type(val), [int, int, str, str]):
      raise RepyArgumentError("Invalid type %s" % type(val))
...
class Float(ValueProcessor):
  ...
  def check(self, val):
    if not _is_in(type(val), [int, int, float]):
      raise RepyArgumentError("Invalid type %s" % type(val))
...
def _copy(obj, objectmap=None):
  ...
  try:
    ...
    if _is_in(type(obj), [str, str, int, int, float, complex, bool, frozenset,
