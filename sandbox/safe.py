...
import util.harshexit  as harshexit  # This is to kill the external process on timeout
import subprocess   # This is to start the external process
try:
  import __builtin__ as builtins
except ImportError:
  import builtins
import sandbox.nonportable as nonportable# This is to get the current runtime
import util.repy_constants as repy_constants# This is to get our start-up directory
import repy_exceptions as exception_hierarchy # For exception classes
...
def safe_check_subprocess(code):
  ...
  path_to_safe_check = os.path.join(repy_constants.REPY_START_DIR, "sandbox", "safe_check.py")
  ...
  # Write out the user code, close so the other end gets an EOF
    ...
    #(rawoutput, _) = proc.communicate(code)
...
#        "{} seconds)".format(nonportable.getruntime() - starttime))
