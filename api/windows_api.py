...
def launch_process(application,cmdline = None, priority = NORMAL_PRIORITY_CLASS):
  ...
  cmdline_param = str(application) + " " + cmdline_param
  application = None
...
def create_mutex(name):
  ...
  handle = _create_mutex(None, 0, str(name))
...
def get_available_interfaces():
  ...
  # Get the keys
  ipconfig_data_keys = list(ipconfig_data.keys())
